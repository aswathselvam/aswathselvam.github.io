...
async function main() {
    ...
    // const url = new URL("https://huggingface.co/aswathselvam/splats/resolve/main/bonsai_trimmed.ksplat");
    // const url = new URL("https://huggingface.co/cakewalk/splat-data/resolve/main/bicycle.splat");
    const url = new URL("https://huggingface.co/VladKobranov/splats/resolve/main/bonsai.splat");
...
canvas.height = 200;
    ...
    const resize = () => {
        ...
        gl.uniform2fv(u_viewport, new Float32Array([wid, hei]));

        // gl.canvas.width = Math.round(wid / downsample);
        // gl.canvas.height = Math.round(hei / downsample);
        gl.canvas.width = Math.round(wid);
        gl.canvas.height = Math.round(hei);
    ...
    //                 0,
    //                 0,
    //                 (-10 * (e.deltaY * scale)) / 250,
    //             );
    //             // inv[13] = preY;
    //         } else {
    //             let d = 4;
    //             inv = translate4(inv, 0, 0, d);
    //             inv = rotate4(inv, -(e.deltaX * scale) / 250, 0, 1, 0);
    //             inv = rotate4(inv, (e.deltaY * scale) / 250, 1, 0, 0);
    //             inv = translate4(inv, 0, 0, -d);
    //         }
    ...
    canvas.addEventListener("mousemove", (e) => {
        ...
        if (down == 1) {
            let inv = invert4(viewMatrix);
            let dx = (5 * (e.clientX - startX)) / 250;
            let dy = (5 * (e.clientY - startY)) / 250;
            let d = 4;
            ...
            inv = translate4(
                inv,
                (-10 * (e.clientX - startX)) / 250,
                0,
                (10 * (e.clientY - startY)) / 250,
            );
            // inv[13] = preY;
            viewMatrix = invert4(inv);
    ...
    canvas.addEventListener(
        ...
        (e) => {
            ...
            if (e.touches.length === 1 && down) {
                let inv = invert4(viewMatrix);
                let dx = (4 * (e.touches[0].clientX - startX)) / 250;
                let dy = (4 * (e.touches[0].clientY - startY)) / 250;
                ...
                inv = translate4(inv, -dx / 250, -dy / 250, 0);
